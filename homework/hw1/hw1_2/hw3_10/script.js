...
for (let h = 0; h < array5.length; h += 1) {
...
for (let z = 0; z < array5.length; z += 1) {
    if (z % 2) {
...
for (let m = 0; m < array5.length; m += 1) {
    let item1 = array5[m];
...
for (let d = array5.length - 1; d >= 0; d -= 1) {
    let item = array5[d];
    if (typeof item === 'number' && !isNaN(item)) {
        if (!(item % 2)) {
...
for (let n = array5.length - 1; n >= 0; n -= 1) {
    let item = array5[n];
    if (typeof item === 'number' && !isNaN(item)) {
        if (!(item % 3)) {
            item = 'okten';
